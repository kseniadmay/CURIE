...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            if not product_size:
                return JsonResponse({'error': 'No size available'}, status=400)

        quantity = form.cleaned_data['quantity']
        if product_size.stock < quantity:
            return JsonResponse({'error': f'Only {product_size.stock} items available'}, status=400)
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error': f"Cannot add {quantity} items. "
                             f"Only {product_size.stock - existing_item.quantity} more available"},
                    status=400)
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({'error': f"Only {cart_item.product_size.stock} items available"}, status=400)
...
class RemoveCartItemView(CartMixin, View):
    def post(self, request, item_id):
        cart = self.get_cart(request, item_id)
        try:
...
class CartCountView(CartMixin, View):
    def get(self, request):
        cart = self.get_cart(request)
        return JsonResponse({'total_items': cart.total_items(), 'subtotal': float(cart.subtotal)})
